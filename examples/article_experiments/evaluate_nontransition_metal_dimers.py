...
ckpt_folder = "checkpoints/ckpts_nontms/"
...
ckpt_dir = os.path.join(data_dirpath, "checkpoint_" + str(checkpoint_step) + "/")
if loadcheckpoint:
...
for molecule in molecules:
    a1, a2 = re.findall('[A-Z][a-z]*', molecule)
    if a1 not in tms and a2 not in tms:
        nontms_molecules.append(molecule)


# make heatmap
fig, ax = heatmap(data = results_array, row_labels = atom_indices, col_labels = atom_indices, molecules=nontms_molecules, colbar = False)
plt.title('(a) Training on dimers devoid of transition metals', fontsize = 18, y = -0.17)


file = os.path.join(data_dirpath, 'dimer_nontm_heatmap.pdf')
...
print('When trained on non-transition metals, the MAE (Ha) for dimers containing / not-containing transition metals is: ', np.round(average_error(predictions, targets)[0],4), np.round(average_error(predictions, targets)[1],4))
