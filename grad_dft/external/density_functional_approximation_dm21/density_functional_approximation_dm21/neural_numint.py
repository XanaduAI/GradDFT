...
class NeuralNumInt(numint.NumInt):
    ...
    def __init__(self, functional: Functional, *, checkpoint_path: Optional[str] = None):
    ...
    def _build_graph(self, batch_dim: Optional[int] = None):
        ...
        rho_only_a, grad_a_x, grad_a_y, grad_a_z, _, tau_a = tf.unstack(rho_a, axis=0)
        rho_only_b, grad_b_x, grad_b_y, grad_b_z, _, tau_b = tf.unstack(rho_b, axis=0)
        ...
        hfxa = tf.placeholder(tf.float32, shape=[batch_dim, len(self._omega_values)], name="hfxa")
        hfxb = tf.placeholder(tf.float32, shape=[batch_dim, len(self._omega_values)], name="hfxb")
        ...
        self._vrho = tf.gradients(
            unweighted_xc,
            [features["rho_a"], features["rho_b"]],
            name="GRAD_RHO",
            unconnected_gradients=tf.UnconnectedGradients.ZERO,
        )
        self._vsigma = tf.gradients(
            unweighted_xc,
            [features["norm_grad_rho_a"], features["norm_grad_rho_b"], features["norm_grad_rho"]],
            name="GRAD_SIGMA",
            unconnected_gradients=tf.UnconnectedGradients.ZERO,
        )
        self._vtau = tf.gradients(
            unweighted_xc,
            [features["tau_a"], features["tau_b"]],
            name="GRAD_TAU",
            unconnected_gradients=tf.UnconnectedGradients.ZERO,
        )
    ...
    def block_loop(
        ...
        for ao, mask, weight, coords in super().block_loop(
            ...
            self._grid_state = _GridState(ao=ao, mask=mask, weight=weight, coords=coords)
    ...
    def eval_xc(
        ...
        verbose=None,
    ) -> Tuple[np.ndarray, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], None, None]:
        ...
        with self._graph.as_default():
            ...
            exc, vrho, vsigma, vtau, vhf = self._session.run(tensor_list, feed_dict=feed_dict)
        ...
        if spin == 0:
            ...
            for i in range(len(self._omega_values)):
                ...
                self._vmat_hf[0] += _dot_ao_ao(mol, ao, aow, mask, shls_slice, ao_loc_nr)
                aow = np.einsum("pi,p->pi", fxxb[:, :, i], -0.5 * vhfs[:, i, 1])
                self._vmat_hf[1] += _dot_ao_ao(mol, ao, aow, mask, shls_slice, ao_loc_nr)
