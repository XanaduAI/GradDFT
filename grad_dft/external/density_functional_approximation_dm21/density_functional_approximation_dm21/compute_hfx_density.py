...
def _evaluate_nu_slow(
    mol: mole.Mole, coords: np.ndarray, omega: float, hermi: int = 1
) -> np.ndarray:
...
def _evaluate_nu(mol: mole.Mole, coords: np.ndarray, omega: float, hermi: int = 1) -> np.ndarray:
...
def _nu_chunk(
    mol: mole.Mole, coords: np.ndarray, omega: float, chunk_size: int = 1000
...
def _compute_exx_block(nu: np.ndarray, e: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
def _compute_jk_block(
    nu: np.ndarray, fxx: np.ndarray, dm: np.ndarray, ao_value: np.ndarray, weights: np.ndarray
