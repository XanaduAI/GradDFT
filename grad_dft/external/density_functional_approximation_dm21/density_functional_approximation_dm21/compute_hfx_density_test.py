...
class ComputeHfxDensityTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        {"testcase_name": "local_hf", "omega": 0.0},
    ...
    def test_closed_shell(self, omega):
        ...
        results = compute_hfx_density.get_hf_density(mol, dm, coords, omega=omega, weights=weights)
    ...
    def test_hf_density_on_open_shell(self, omega):
        ...
        with mol.with_range_coulomb(omega=omega):
            target_j, target_k = scf.hf.get_jk(mol, dm)
        target_hf = -0.5 * (
            np.einsum("ij,ji", dm[0], target_k[0]) + np.einsum("ij,ji", dm[1], target_k[1])
        )
        ...
        predicted_hf = np.sum((results.exx[0] + results.exx[1]) * weights)
        coulomb = np.einsum("ij,ji", dm[0], results.coulomb[0]) + np.einsum(
            "ij,ji", dm[1], results.coulomb[1]
        )
        hf = -0.5 * (
...
def _nu_test_systems():
    systems = [
        {"atom": "N 0 0 0; N 0 0 2.4", "charge": 0, "spin": 0, "basis": "cc-pVDZ", "num_grids": -1},
        {"atom": "N 0 0 0; N 0 0 2.4", "charge": 0, "spin": 0, "basis": "cc-pVDZ", "num_grids": 1},
        {"atom": "N 0 0 0; N 0 0 2.4", "charge": 0, "spin": 0, "basis": "cc-pVDZ", "num_grids": 2},
        {"atom": "N 0 0 0; N 0 0 2.4", "charge": 0, "spin": 0, "basis": "cc-pVDZ", "num_grids": 10},
        {"atom": "N 0 0 0; N 0 0 2.4", "charge": 0, "spin": 0, "basis": "cc-pVDZ", "num_grids": 32},
        {"atom": "N 0 0 0; N 0 0 2.4", "charge": 0, "spin": 0, "basis": "cc-pVDZ", "num_grids": 33},
        {"atom": "Li 0 0 0", "charge": 0, "spin": 1, "basis": "cc-pVTZ", "num_grids": -1},
        {"atom": "H 0 0 0", "charge": 0, "spin": 1, "basis": "cc-pVQZ", "num_grids": -1},
    ]
...
class NuTest(parameterized.TestCase):
    ...
    def test_nu_integrals(self, atom, charge, spin, basis, num_grids, hermi):
        ...
        nu_slow = compute_hfx_density._evaluate_nu_slow(mol, test_coords, omega=0.0, hermi=hermi)
        nu_fast = compute_hfx_density._evaluate_nu(mol, test_coords, omega=0.0, hermi=hermi)
    ...
    def test_range_separated_nu(self):
        ...
        expected_result = np.squeeze(np.einsum("g,ga,gb->ab", weights * erf, ao_value, ao_value))
