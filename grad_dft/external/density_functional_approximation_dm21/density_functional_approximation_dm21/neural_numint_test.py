...
class NeuralNumintTest(tf.test.TestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        ...
        {"functional": neural_numint.Functional.DM21mc, "expected_energy": -37.33489173},
        {"functional": neural_numint.Functional.DM21mu, "expected_energy": -37.34015315},
    )
    ...
    def test_exported_model(self):
        ...
        tensor_inputs = {
            k: tf.constant(v, dtype=tf.float32) for k, v in attr.asdict(inputs).items()
        }
        ...
        self.assertAllClose(outputs, exported_outputs, atol=5.0e-5, rtol=1.0e-5)
