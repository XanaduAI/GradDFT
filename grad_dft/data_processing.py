...
def process_atoms(training=True, combine=False, max_cycle=None, charge=0, noise=0):
    ...
    for i in tqdm(range(1, 37), desc="Processing atoms"):
        ...
        if ELEMENTS.index(atom) <= charge:
            continue
        ghost_atom = ELEMENTS[i - charge]
        spin = compute_spin_element(
            ghost_atom
        )  # In case the atom is charged, the configuration changes
        charge = charge
        geometry = [[atom, [0, 0, 0]]]
...
########### Execute them #############
# process_w4x17()
for functional in tqdm(
    ["pbe0", "revTPSSh", "MN15", "M06L", "M06_2X", "SCAN", "wB97X_D", "wB97X_V", "TPSS"],
    desc="####### XC functionals #######",
):
